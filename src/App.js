...
function calcPlates(
  ...
  totalWeight = 20, // entered weight to calculate
  barWeight = 20, // standard bar weight is 45 lbs
  clips = 2.5,
  availablePlates = [2.5, 5, 10, 15, 20, 25] // plates at my gym
) {
  // Declaring variables
  let result = [], // stores an array of plates required
    ...
    load = ((totalWeight / 2.2) - barWeight - clips) / 2; //calculates weight minus the bar weight
...
function calcWarmUp(totalWeight, barWeight = 20) {
  let warmUpOne = barWeight;
  let warmUpTwo = Math.floor(((totalWeight/2.2) * 0.45) / 5) * 5;
  let warmUpThree = Math.floor(((totalWeight/2.2) * 0.65) / 5) * 5;
  let warmUpFour = Math.floor(((totalWeight/2.2) * 0.85) / 5) * 5;
  ...
  arrWarmUp.forEach((elem, ind) => {
    ...
    warmUpWeightEntry.innerHTML = `<b>${elem} kg</b> (${arrWarmUpPlates[ind]}) x <b>${arrWarmUpReps[ind]} reps</b>`;
